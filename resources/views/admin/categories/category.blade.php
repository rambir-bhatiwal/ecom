			...
			<div class="ps-3">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb mb-0 p-0">
						<li class="breadcrumb-item"><a href="{{ url('/admin/categories') }}"><i class="bx bx-home-alt"></i></a>
						</li>
									...
									<div class="col-sm-12">
										<table id="example2" class="table table-striped table-bordered dataTable" role="grid" aria-describedby="example2_info">
											...
											<tbody>
												...
												<tr role="row" id="{{ $category->id }}" class="@if($category->id % 2) odd @else even @endif">
													<td class="sorting_1">{{ $category->id }}</td>	
													<td class="sorting_1">{{ $category->name }}</td>	
													<!-- <td class="title">{# $category->category->name #}</td> -->
													<td class="title" >{{ $category->slug }}</td>
													<td> <button type="button" class="btn btn-outline-info px-5 radius-30" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="{{ $category->id }}"  onclick="editCategory('{{ $category }}');">Edit</button> </td>
													<td> <button type="button" class="btn btn-outline-danger px-5 radius-30"  data-id="{{ $category->id }}"  onclick="deleteCategory('{{ $category->id }}');">Delete</button> </td>
												@endforeach
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="color-form" method="post" action="{{ url('/admin/categories/store') }}" enctype="multipart/form-data">
					@csrf
					<div class="mb-3">
					...
					<div class="mb-3">
						...
						<select name="parent_id" id="parent_category_id" class="form-control">
							<option id="parent-id-none" value="">Select Parent Category</option>
							@foreach ( $categories as $category )
							<option id="parent-id-{{ $category->id }}" value="{{ $category->id }}">{{ $category->name }}</option>
							@endforeach
						</select>
					</div>
				</form>
...
<script>
	...
	function deleteCategory(id) {
		var xhr = new XMLHttpRequest();
		console.log(id);
		xhr.open('GET', '/admin/categories/delete/' + id, true);
		xhr.onload = function() {
			let data = JSON.parse(xhr.responseText);
			if (xhr.status === 200) {
				if (data.status === 200) {
	...
	function editCategory(data) {
		...
		if (JSON.parse(data).parent_id && JSON.parse(data).parent_id > 0) {
			document.getElementById('parent-id-'+JSON.parse(data).parent_id).selected  = true;
		}else{
			document.getElementById('parent-id-none').selected  = true;
		}
