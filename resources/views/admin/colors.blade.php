			...
			<div class="ps-3">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb mb-0 p-0">
						<li class="breadcrumb-item"><a href="{{ url('/admin/colors') }}"><i class="bx bx-home-alt"></i></a>
						</li>
										...
										<table id="example2" class="table table-striped table-bordered dataTable" role="grid" aria-describedby="example2_info">
											<thead>
												<tr role="row">
													...
													<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 275.975px;">Color</th>
											...
											<tbody>
												...
												<tr role="row" id="{{ $color->id }}" class="@if($color->id % 2) odd @else even @endif">
													...
													<td class="title" style="background-color: {{ $color->value }}">{{ $color->value }}</td>
													<td> <button type="button" class="btn btn-outline-info px-5 radius-30" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="{{ $color->id }}" data-val="{{ $color->name }}" onclick="editColors('{{ $color }}');">Edit</button> </td>
													<td> <button type="button" class="btn btn-outline-danger px-5 radius-30"  data-id="{{ $color->id }}" data-val="{{ $color->name }}" onclick="deleteColors('{{ $color->id }}');">Delete</button> </td>
												@endforeach
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="color-form" method="post" action="{{ url('/admin/colors/store') }}" enctype="multipart/form-data">
					@csrf
					<div class="mb-3">
			...
			<div class="modal-footer">
				...
				<button type="submit" class="btn btn-primary" id="modal-submit-button" form="color-form">Add color</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	function deleteColors(id) {
		var xhr = new XMLHttpRequest();
		xhr.open('GET', '/admin/colors/delete/' + id, true);
		xhr.onload = function() {
	...
	function editColors(data) {
		...
		document.getElementById('modal-submit-button').innerText = 'Update Color';
		document.querySelector('.modal-title').innerText = 'Edit Color';
	}
</script>